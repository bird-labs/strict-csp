...
var StrictCsp = /** @class */ (function () {
     ...
     */
    StrictCsp.getStrictCsp = function (hashes, 
    // default CSP options
    cspOptions) {
        ...
        var strictCspTemplate = {
            ...
            'script-src': __spreadArray(["'strict-dynamic'"], hashes),
    ...
    StrictCsp.prototype.refactorSourcedScriptsForHashBasedCsp = function () {
        var _this = this;
        var scriptTagList = this.$(StrictCsp.SOURCED_SCRIPT_SELECTOR)
            .map(function (i, script) {
            _this.$(script).remove();
            return script;
        })
            .filter(function (script) { return _this.$(script).attr("src") !== null; })
            .get();
        var scripts = scriptTagList.map(function (scriptTag) {
            var script = {};
            var scriptElementProperties = ['type', 'src', 'event', 'charset', 'async', 'defer', 'crossOrigin', 'text', 'fetchPriority', 'noModule', 'referrerPolicy'];
            scriptElementProperties.forEach(function (propertyName) {
                var scriptPropertyValue = _this.$(scriptTag).attr(propertyName);
        ...
        var loaderScript = StrictCsp.createLoaderScript(scripts);
        if (!loaderScript) {
            return;
        }
    ...
    StrictCsp.prototype.hashAllInlineScripts = function () {
        ...
        return this.$(StrictCsp.INLINE_SCRIPT_SELECTOR)
            .map(function (i, elem) { return StrictCsp.hashInlineScript(_this.$(elem).html() || ''); })
            .get();
    };
    /**
     ...
     */
    StrictCsp.createLoaderScript = function (scripts) {
     ...
     */
    StrictCsp.hashInlineScript = function (scriptText) {
        var hash = crypto
